...
class _MedicalRecordHomeScreenState extends State<MedicalRecordHomeScreen> {
  ...
  void didChangeDependencies() {
    ...
    _loadMedicalRecord(languageCode);
  }

  Future<void> _loadMedicalRecord(String languageCode) async {
    debugPrint('🟦 MedicalRecordHomeScreen load started');

    try {
      final result = await MedicalRecordDi.getAllPrescriptionsUseCase(
        languageCode: languageCode,
      );

      debugPrint('🟦 Prescriptions loaded: ${result.data.length}');
      debugPrint('🟦 Prescriptions from cache: ${result.isFromCache}');
      ...
      debugPrint('🟥 Prescriptions error: $error');
    ...
    debugPrint('🟦 MedicalRecordHomeScreen load finished');
  }  @override
              ...
              children: [
                Expanded(
                  child: MedicalRecordCategoryCard(
                    title: 'وصفات',
                    subtitle: '${_prescriptions.length} وصفات',
                ...
                Expanded(
                  child: MedicalRecordCategoryCard(
                    title: 'دفعات',
                    subtitle:
                    _paymentPlan?.remainingAmountLabel ?? 'بدون بيانات',
                    icon: Icons.wallet_rounded,
                    onTap: () {
                      context.push(AppRoutes.medicalRecordPayments);
                    },
                  ),
                ),
              ],
